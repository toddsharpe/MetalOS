...
//Internal protypes
int kvwprintf(CHAR16 const* fmt, void (*func)(int, void*), void* arg, int radix, va_list ap);
CHAR16* kswprintn(CHAR16* nbuf, uintmax_t num, int base, int* len, int upper);
...
CHAR16 const hex2char16_data[] = L"0123456789abcdefghijklmnopqrstuvwxyz";

#define hex2ascii(hex)  (hex2char16_data[hex])
...
int vwprintf(CHAR16* const _Buffer, CHAR16 const* const _Format, va_list _ArgList)
{
	int retval;
 ...
 */
CHAR16* kswprintn(CHAR16* nbuf, uintmax_t num, int base, int* lenp, int upper)
{
	CHAR16* p, c;
