...
#define hex2ascii_w(hex) (hex2wchar_t_data[hex])
#define toupper_w(c) ((c) - 0x20 * (((c) >= L'a') && ((c) <= L'z')))

inline int imax_w(int a, int b)
{
	return (a > b ? a : b);
}
...
wchar_t *kswprintn(wchar_t *nbuf, uintmax_t num, int base, int *lenp, int upper)
{
	...
	*p = L'\0';
	do
	{
		c = hex2ascii_w(num % base);
		*++p = upper ? toupper_w(c) : c;
	} while (num /= base);
	if (lenp)
		*lenp = (int)(p - nbuf);
	return (p);
...
#define PCHAR(c)              \
	...
	reswitch:
		...
		case L'D':
			...
			while (width--)
			{
				PCHAR(hex2ascii_w(*up >> 4));
				PCHAR(hex2ascii_w(*up & 0x0f));
				up++;
				if (width)
					for (q = p; *q; q++)
						PCHAR(*q);
