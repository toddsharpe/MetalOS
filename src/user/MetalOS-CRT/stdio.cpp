#pragma once

#include "user/MetalOS.h"
#include <stdio.h>
#include "Assert.h"

typedef void* FILE;
/* Seek method constants */

#define SEEK_CUR    1
#define SEEK_END    2
#define SEEK_SET    0

/* Buffered I/O macros */

#define BUFSIZ  512

extern "C"
{
	FILE* fopen(char const* _FileName, char const* _Mode)
	{
		//inspect mode
		return reinterpret_cast<FILE*>(CreateFile(_FileName, FileAccess::Read));
	}

	size_t fread(void* _Buffer, size_t _ElementSize, size_t _ElementCount, FILE* _Stream)
	{
		if (!_ElementSize || !_ElementCount)
			return 0;
		
		size_t bytes;
		SyscallResult result = ReadFile(reinterpret_cast<HFile>(_Stream), _Buffer, _ElementSize * _ElementCount, &bytes);
		Assert(result == SyscallResult::Success);

		return bytes / _ElementSize;
	}

	size_t fwrite(void const* _Buffer, size_t _ElementSize, size_t _ElementCount, FILE* _Stream)
	{
		if (!_ElementSize || !_ElementCount)
			return 0;

		size_t bytes;
		AssertSuccess(WriteFile(reinterpret_cast<HFile>(_Stream), _Buffer, _ElementSize * _ElementCount, &bytes));

		return bytes / _ElementSize;
	}

	int fseek(FILE* _Stream, long _Offset, int _Origin)
	{
		enum Seek move;
		switch (_Origin)
		{
		case SEEK_SET:
			move = Seek::Begin;
			break;

		case SEEK_CUR:
			move = Seek::Current;
			break;

		case SEEK_END:
			move = Seek::End;
			break;

		default:
			return -1;
		}
		
		size_t newPos;

		SyscallResult result = SetFilePointer(reinterpret_cast<HFile>(_Stream), _Offset, move, &newPos);
		return (int)newPos;
	}

	long ftell(FILE* _Stream)
	{
		return fseek(_Stream, 0, SEEK_CUR);
	}

	int fclose(FILE* _Stream)
	{
		return static_cast<int>(CloseHandle(reinterpret_cast<HFile>(_Stream)));
	}

	//TODO: make this real, it just prints to debug stream
	//TODO: buffering?
	int vfprintf(FILE* const _Stream, char const* const _Format, va_list _ArgList)
	{
		char buffer[BUFSIZ];
		int retval = vsprintf(buffer, _Format, _ArgList);
		buffer[retval] = '\0';

		AssertSuccess(WriteFile(reinterpret_cast<HFile>(_Stream), buffer, retval + 1, nullptr));
		return retval + 1;
	}

	int fprintf(FILE* const _Stream, char const* const _Format, ...)
	{
		va_list args;
		va_start(args, _Format);
		int result = vfprintf(_Stream, _Format, args);
		va_end(args);
		return result;
	}

	int fflush(FILE *stream)
	{
		//Do nothing
		return 0;
	}

	int rename(char const* _OldFileName, char const* _NewFileName)
	{
		return static_cast<int>(MoveFile(_OldFileName, _NewFileName));
	}

	int remove(char const* _FileName)
	{
		return static_cast<int>(DeleteFile(_FileName));
	}

	int printf(char const* const _Format, ...)
	{
		va_list args;
		va_start(args, _Format);
		int result = vfprintf((FILE*)stdout, _Format, args);
		va_end(args);
		return result;
	}

	int puts ( const char * str )
	{
		const size_t length = strlen(str);
		AssertSuccess(WriteFile(reinterpret_cast<HFile>(stdout), str, length + 1, nullptr));
		putchar('\n');
		return 0;
	}

	int putchar ( int character )
	{
		AssertSuccess(WriteFile(reinterpret_cast<HFile>(stdout), &character, 1, nullptr));
		return 0;
	}

	int sscanf ( const char * s, const char * format, ...)
	{
		//Not implemented
		return -1;
	}
}
