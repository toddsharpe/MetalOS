...
PageTablesPool::PageTablesPool(void *const virtualBase, const paddr_t physicalBase, const size_t count) : m_virtualBase(reinterpret_cast<uintptr_t>(virtualBase)),
...
bool PageTablesPool::AllocatePage(paddr_t &address)
{
	...
	for (size_t i = 1; i < this->m_count; i++)
	{
		...
		// Allocate page
		m_index[i] = true;
		address = (m_physicalBase + (i << x64::PageShift));
		return true;
	}
...
{
	Assert(address % x64::PageSize == 0);

	const paddr_t relative = (address - m_physicalBase);
	const size_t index = relative >> x64::PageShift;
